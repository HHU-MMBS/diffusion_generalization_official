...
def training_loop(
    ...
    dataset_obj = edm2.dnnlib.util.construct_class_by_name(**dataset_kwargs)
    ref_image, ref_label = dataset_obj[0]
    dist.print0('Setting up encoder...')
    encoder = edm2.dnnlib.util.construct_class_by_name(**encoder_kwargs)
    ...
    net = edm2.dnnlib.util.construct_class_by_name(**network_kwargs, **interface_kwargs)
    net.train().requires_grad_(True).to(device)

    # Load main network.
    if load is not None:
        with edm2.dnnlib.util.open_url(load) as f:
            data = pickle.load(f)  # keys: ema, encoder, dataset_kwargs, loss_fn
        dist.print0('\n \n NEW: Loading pretrained network...', load)
        misc.copy_params_and_buffers(src_module=data['ema'], dst_module=net, require_all=False)
        # decision 1: load loss function from checkpoint
        loss_fn = data['loss_fn']
        # loss_fn = dnnlib.util.construct_class_by_name(**loss_kwargs)

        # decision 2: load loss function from checkpoint
        # cur_nimg=0

        cur_nimg = int(load.split("-")[-2])*1000
        # make cur_nimg multiple of batch size
        cur_nimg = cur_nimg - (cur_nimg % batch_size)
        del data
    else:
        cur_nimg = 0
        loss_fn = edm2.dnnlib.util.construct_class_by_name(**loss_kwargs)
    ...
    state = dnnlib.EasyDict(cur_nimg=cur_nimg, total_elapsed_time=0)
    ddp = torch.nn.parallel.DistributedDataParallel(net, device_ids=[device])
    # loss_fn = dnnlib.util.construct_class_by_name(**loss_kwargs)
    optimizer = edm2.dnnlib.util.construct_class_by_name(params=net.parameters(), **optimizer_kwargs)
    ema = edm2.dnnlib.util.construct_class_by_name(net=net, **ema_kwargs) if ema_kwargs is not None else None
    dist.print0(optimizer.param_groups[0]["weight_decay"])
    ...
    dist.print0(f'Status  {status_nimg // 1000} kimg. Save every {snapshot_nimg // 1000} kimg')
    dist.print0(f'checkpoint_nimg  {checkpoint_nimg // 1000} kimg')
    ...
    dataset_iterator = iter(
        edm2.dnnlib.util.construct_class_by_name(dataset=dataset_obj, sampler=dataset_sampler, batch_size=batch_gpu, **data_loader_kwargs))
    ...
    while True:
        ...
        if status_nimg is not None and (done or state.cur_nimg % status_nimg == 0) and (state.cur_nimg != start_nimg or start_nimg == 0):
            ...
            dist.print0(' '.join(['Status:',
                ...
                'time',         f"{edm2.dnnlib.util.format_time(training_stats.report0('Timing/total_sec', state.total_elapsed_time)):<12s}",
            ...
            if dist.get_rank() == 0:
                ...
                fmt = {'Progress/tick': '%.0f', 'Progress/kimg': '%.3f', 'timestamp': '%.3f', "loss": '%.3f'}
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                assert not torch.any(torch.isnan(loss)), 'Training stopped because loss was NaN'

        # Run optimizer and update weights.
        lr = edm2.dnnlib.util.call_func_by_name(cur_nimg=state.cur_nimg, batch_size=batch_size, **lr_kwargs)
        training_stats.report('Learning_rate', lr)
