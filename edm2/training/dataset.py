...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        label_dim           = None,     # Specify label_dim, otherwise interpreted from give labels
    ):
        self._name = name
    ...
    def get_label(self, idx):
        if self._use_pseudo_labels:
            ...
            label_dim = self.label_dim if self.label_dim is not None else self.get_label_dim()
            if np.random.rand() > self.p_uncond:
                return torch.nn.functional.one_hot(label.long(), num_classes=label_dim).long()
            else:
                return torch.zeros(label_dim, dtype=torch.long)
