...
def cmdline():
    ...
    python frechet_utils.py calc --images=out \\
    ...
    torchrun --standalone --nproc_per_node=8 frechet_utils.py gen \\
    ...
    python frechet_utils.py ref --data=datasets/my-dataset.zip \\
...
def calc(ref_path, metrics, **opts):
    ...
    if dist.get_rank() == 0:
        ...
        out_path = opts["image_path"] + ".csv"
        pd.DataFrame(results, index=[0]).to_csv(out_path)        
